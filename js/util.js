 ...
 * @return       [description]
 */
export const queryParse = (value, options = { sep: '&', eq: '=', isDecode: true }) => {
  const decode = options.isDecode ? decodeURIComponent : (a) => a
 ...
 * @return     [description]
 */
export const queryStringify = (obj = {}, options = { sep: '&', eq: '=', isEncode: true }) => {
  const encode = options.isEncode ? encodeURIComponent : (a) => a
 ...
 * 文字列のバイト数配列取得
 */
export const getByteLengthArray = (str) => {
  if (!str) return []
 ...
 * 最大ページ数取得
 */
export const getMaxPage = (total, count) => {
  return total % count
